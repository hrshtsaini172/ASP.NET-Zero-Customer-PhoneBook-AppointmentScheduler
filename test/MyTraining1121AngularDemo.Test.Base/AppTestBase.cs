...
using MyTraining1121AngularDemo.Test.Base.TestData;
...
namespace MyTraining1121AngularDemo.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="MyTraining1121AngularDemoDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(MyTraining1121AngularDemoDbContext context)
            {
        ...
        protected void UsingDbContext(Action<MyTraining1121AngularDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyTraining1121AngularDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<MyTraining1121AngularDemoDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<MyTraining1121AngularDemoDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyTraining1121AngularDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyTraining1121AngularDemoDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyTraining1121AngularDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyTraining1121AngularDemoDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<MyTraining1121AngularDemoDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<MyTraining1121AngularDemoDbContext, Task<TResult>> func)
        {
            TResult result;
